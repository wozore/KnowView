...
const { isModelKey, normalizeModelIdentity } = require('../../shared/model-key-contract');
...
function identityKey(value, label) {
  const text = String(value || '').trim().normalize('NFKC').replace(/[^A-Za-z0-9.\s_-]+/g, '-');
  try { return normalizeModelIdentity(text); } catch { throw new Error('KEY_REQUIRED:' + label); }
}
