...
| 字段 | 默认值 | 说明 |
|---|---|---|
| `youtube_cron` | `"13 12 * * *"` | YouTube 抓取 cron（UTC 值；意图北京时间每天 20:13 触发，管线内 72h 到期闸决定是否真正采集）。不用 `*/3` 的原因：`*/3` 是月历日（1,4,…,28,31），月末出现 31→1 背靠背 |
...
| `x_cron_hot` | `"37 0 * * *"` | X 热半区抓取 cron（UTC 值；北京时间每天 08:37 触发，覆盖前一日 20:00 至当日 08:00 发布高峰） |
| `x_cron_cold` | `"43 12 * * *"` | X 冷半区抓取 cron（UTC 值；北京时间每天 20:43 触发，覆盖当日 08:00 至 20:00 白天发布） |
| `x_tz` | `"Asia/Shanghai"` | X 抓取意图时区（北京时间） |

X 采集窗口按方案 A 双半区连续覆盖：
- **热半区**：覆盖北京时间 `[前一日 20:00, 当日 08:00)`，08:37 启动；
- **冷半区**：覆盖北京时间 `[当日 08:00, 当日 20:00)`，20:43 启动；
两次采集窗口连续无重叠，去重后合并入单状态轴候选层。调度触发时每次只采 X，不采 YouTube。
三个 cron 均错开整点/半点（GitHub 官方文档警告 :00/:30 是全球调度最高峰，2026-09-14/15 曾实测该时段延迟 4.6~10 小时且整次触发被丢弃）；窗口关闭到运行的 30 分钟缓冲相应放宽为 37/43 分钟，仅为运营缓冲，不构成"不漏采"保证。
