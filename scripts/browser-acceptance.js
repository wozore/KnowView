...
async function main() {
  ...
  try {
    ...
    await wait(300);
    await assertBrowser(client, '17 张模型卡逐一可搜索', `(()=>{const targets=['GLM-5.3','Kimi K3','Qwen3.8 Max','Muse Spark 1.2','Gemini 3.7 Flash','Gemini 3.6 Flash','MiniMax M3','MiniMax M2.7','Grok 4.5','混元 Hy3','Step-3.5-Flash','Step-3.7-Flash','Nemotron 3 Ultra','Nemotron 3 Super','Nemotron 3.5','MiMo-V2.5','MiMo-V2.5 Pro'];const input=document.querySelector('#searchInput');const root=document.querySelector('#toolDirectoryView');return (async()=>{for(const target of targets){input.value=target;input.dispatchEvent(new Event('input',{bubbles:true}));await new Promise(resolve=>setTimeout(resolve,80));if(!root.innerText.includes(target))return false;}return true})()})()`);
    await assertBrowser(client, 'Gemini CLI 显示最近更新', `(()=>{const input=document.querySelector('#searchInput');input.value='Gemini CLI';input.dispatchEvent(new Event('input',{bubbles:true}));const name=[...document.querySelectorAll('.tool-card-name')].find(x=>['Gemini CLI','Gemini CLI（具体模型）'].includes(x.textContent.trim()));const card=name?.closest('.tool-card');if(!card)return false;card.click();const valid=/最近更新 \\d{4}-\\d{2}-\\d{2}/.test(document.querySelector('#modalContent').innerText);window.closeModal();return valid})()`);
    await assertBrowser(client, 'Kimi K3 显示发布日期', `(()=>{const input=document.querySelector('#searchInput');input.value='Kimi K3';input.dispatchEvent(new Event('input',{bubbles:true}));const name=[...document.querySelectorAll('.tool-card-name')].find(x=>['Kimi K3','Kimi K3（具体模型）'].includes(x.textContent.trim()));const card=name?.closest('.tool-card');if(!card)return false;card.click();const valid=/发布日期 \\d{4}-\\d{2}-\\d{2}/.test(document.querySelector('#modalContent').innerText);window.closeModal();return valid})()`);
