...
async function main() {
  ...
  try {
    ...
    for (const [pickName, pick] of [['发布日期', datePicks && datePicks.release], ['最近更新', datePicks && datePicks.updated]]) {
      ...
      await assertBrowser(client, `${pickName} typed 标签正确：${pick.title}`, `(()=>{const input=document.querySelector('#searchInput');input.value=${JSON.stringify(pick.title)};input.dispatchEvent(new Event('input',{bubbles:true}));const name=[...document.querySelectorAll('.tool-card-name')].find(x=>x.textContent.trim().startsWith(${JSON.stringify(pick.title)}));const card=name&&name.closest('.tool-card');if(!card)return false;card.click();const valid=document.querySelector('#modalContent').innerText.includes(${JSON.stringify(expectedText)});window.closeModal();return valid})()`);
    }
