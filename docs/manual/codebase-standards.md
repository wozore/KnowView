...
8. **文档治理与生命周期**：
   ...
   - **过时信息覆盖式修正**：文档正文（手册、契约、索引、配置示例、bat 提示、代码注释）出现与代码、登记表或数据现状不符的陈述时，必须**原地删除并改写为当前事实**；严禁以“已过时”“此前为”“legacy”“（已废弃）”等标记形式把旧陈述留在原地供引用。历史过程与演进叙述只写入 `docs/manual/dev-log.md`。ADR 等决策记录以决策当时语境书写过去时事实，头部状态行只写当前事实，不得保留被推翻的现在时断言。修订以代码与登记表为唯一事实源。
   - **契约修改需先获许可**：需求、架构、数据 Schema、系列政策、生命周期、验证门禁、协作流程或角色职责等项目契约，只有在获得用户明确许可后才能修改；测试、数据或实现不得绕过未解决的契约决策。
